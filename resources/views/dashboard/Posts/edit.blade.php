            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div
                        ...
                        <h6 class="text-white text-capitalize m-0">Update Post</h6>
                    </div>
                </div>
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive p-0">
                        <form action="/dashboard/posts/{{ $post->id }}" enctype="multipart/form-data" method="post" class="px-4 pb-4">
                            @method('patch')
                            @csrf
                            ...
                            <div class="input-group mb-3 input-group-outline">
                                <textarea class="form-control" placeholder="Description" type="text" id="description" name="description">{{ $post->description }}</textarea>
                            </div>
                            @error('description')
                                <p class="text-danger">{{ $message }}</p>
                            @enderror
